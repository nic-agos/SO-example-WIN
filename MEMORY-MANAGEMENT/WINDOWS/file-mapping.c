...
{
	char *p;
	p = (char *) MapViewOfFile(mapping, FILE_MAP_WRITE,  0, 0, 0);  //collego al mio address space la memoria condivisa
	if (p == NULL) {
		Errore_("MapViewOfFile error");
	}
	puts("Type the strings to post to file mapping ('quit' to close):");
	fflush(stdout);

	do {
		scanf("%s", messaggio);
		strncpy(p, messaggio, DISP_); //copy una quantit� pari a 20 byte 
		p += DISP_;
	} while( (strcmp(messaggio,"quit") != 0)); //aggiungere il controllo sullo spazio rimanente
...
int main(int argc, char *argv[])
{
	...
	{
		mapping = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, 50000, "my_mapping");  //creo lo storage condivisibile a contenuto anonimo con nome my_mapping
		...
		if (argc == 2 && strcmp(argv[1], "scrittore") == 0){
			mapping = OpenFileMapping(FILE_MAP_WRITE, FALSE, "my_mapping"); /* Apro il file mapping usando il nome*/
